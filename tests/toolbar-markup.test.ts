...
test('contextual toolbar uses the approved groups, action order, icons, and label policy', () => {
  ...
  assert.deepEqual(controls.map((button) => (
    ...
    'toggle-insert', 'replace-image', 'delete-block', 'save', 'done',
  ]);
  assert.deepEqual(controls.map((button) => button.querySelector('svg')?.getAttribute('data-icon')), [
    ...
    'plus', 'image-plus', 'trash-2', 'save', 'check',
  ]);
  ...
  assert.deepEqual(controls.filter((button) => !button.classList.contains('icon-only')).map((button) => button.textContent?.trim()), [
    'Paragraph', 'Insert', 'Replace image', 'Delete', 'Save', 'Done',
  ]);
...
test('text style, insert, link, state, and tooltip markup expose names and reserved actions', () => {
  ...
  assert.deepEqual(insertItems.map((button) => button.textContent?.trim()), ['Paragraph below', 'Image', 'Video']);
  assert.deepEqual(insertItems.map((button) => button.querySelector('svg')?.getAttribute('data-icon')), ['pilcrow', 'image-plus', 'video']);
  ...
  assert.equal(insertItems[1].dataset.action, 'open-image');
  assert.equal(insertItems[2].disabled, false);
  assert.equal(insertItems[2].dataset.action, 'open-video');
  ...
  assert.deepEqual(
    ...
    ['Upload video', 'Insert video', 'Cancel'],
  );
