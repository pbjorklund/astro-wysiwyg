...
Astro WYSIWYG supports static paragraphs, headings, bulleted and numbered lists, blockquotes, fenced code blocks, dividers, common inline formatting, links, raster image uploads, and supported frontmatter values. Markdown and MDX article bodies use the same in-place editor. Rendered metadata opens the frontmatter dialog when its text maps uniquely to a scalar field or an item in a string list. The same explicit content registry drives context filtering, default values, source serialization, replacement compatibility, toolbar metadata, and the image, video, and iframe dialog registrations.
